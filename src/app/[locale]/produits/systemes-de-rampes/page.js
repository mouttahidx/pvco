...
export async function generateMetadata() {
  const t = await getTranslations("RailingsPage");
...
export default function Railings() {
  const t = useTranslations("RailingsPage");
  const p = useTranslations("RailingsModels");
  const models_images = [
    "/img/railing/sunspace-traditional-picket-railing-banner-2.jpg",
    "/img/railing/sunspace-traditional-glass-railing-banner-2.jpg",
    "/img/railing/sunspace-contemporary-glass-railing-banner-1.jpg",
    "/img/railing/sunspace-integrated-railing-screen-banner-1.jpg",
  ];
  return (
    <>
      {/* hero  */}
      <section
        ...
        style={{
          backgroundImage:
            "url('/img/railing/sunspace-contemporary-glass-railing-banner-1.jpg')",
        }}
      >
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto flex justify-between flex-col items-center">
        ...
        <p className="mt-6 max-w-5xl text-center mx-auto">{t("1_subtitle")}</p>

        <div className="grid md:grid-cols-2 gap-10 mt-16">
          {[...Array(4)].map((_, i) => (
            <Product
              key={i}
              link={data.railing_models[i]}
              img={models_images[i]}
